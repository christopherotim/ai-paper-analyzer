...
# 方式二：命令行运行
.\安装环境.bat     # 自动检测Python、创建虚拟环境、安装依赖
```
...
```bash
# 环境问题一键修复
双击 安装环境.bat        # 重新安装和配置环境

# 快速启动环境
双击 启动环境.bat        # 激活虚拟环境并提供使用指导
...
| 工具                  | 功能                   | 使用场景               |
| --------------------- | ---------------------- | ---------------------- |
| `安装环境.bat`        | 一键安装所有依赖和配置 | 首次安装或重新配置环境 |
| `启动环境.bat`        | 快速激活虚拟环境       | 日常使用前的环境激活   |
| `检查环境.py`         | 智能诊断系统状态       | 排查问题或验证安装     |
| `虚拟环境使用指南.md` | 详细的环境管理文档     | 深入了解环境管理       |

**💡 环境管理最佳实践**：

- 🎯 **新手用户**: 直接双击 `安装环境.bat` → `启动环境.bat` → 开始使用
